/* global React, Icon, Crest, Sunburst, Corners, Wordmark, CENIT_DATA, formatCOP, romanDate, ROMAN, toRoman */
const { useState: useStateHome, useEffect: useEffectHome } = React;

// ============================================================
// INICIO — Concierge desk / private club lobby (NOT a landing page).
// Compact welcome panel with today's overview + quick actions.
// ============================================================
function ViewInicio({ go }) {
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-10">
        <div>
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="font-mono text-xs" style={{ color: '#8B6F3F' }}>BOGOTÁ · 18°C</span>
          </div>
          <h1 className="font-display text-5xl lg:text-7xl leading-[1.05]" style={{ color: '#F1ECDE', fontStyle: 'italic', fontWeight: 400 }}>
            Bienvenido al<br/>
            <span className="text-gold not-italic font-roman" style={{ fontSize: '0.65em', letterSpacing: '0.18em', fontWeight: 600 }}>CLUB CÉNIT</span>
          </h1>
          <p className="font-mono text-xs mt-4 max-w-md" style={{ color: '#948A78' }}>
            EST. MMXXIV · CRA·11 №94—32 · CHICÓ · BOGOTÁ
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="ticket">
            ...
            <div className="space-y-2 text-sm">
              <Row label="Cliente"  value={next.client}/>
              <Row label="Servicio" value={next.service}/>
              <Row label="Barbero"  value={next.barber}/>
            </div>
            <button onClick={() => go('reservar')} className="mt-6 font-mono text-[11px] inline-flex items-center gap-2" style={{ color: '#E8C77E' }}>
        ...
        <div className="lg:col-span-12 mt-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <button onClick={() => go('reservar')} className="font-mono text-[11px] inline-flex items-center gap-2" style={{ color: '#948A78' }}>
              VER TODOS <Icon name="ArrowRight" size={12}/>
            </button>
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
            {CENIT_DATA.services.slice(0, 6).map((s, i) => (
              <button key={s.id} onClick={() => go('reservar', { preselectService: s.id })}
        ...
        <div className="lg:col-span-12 mt-8 corner-deco frame-double p-8 lg:p-12 grid lg:grid-cols-3 gap-8">
          ...
          <Pillar n="III" t="Garantía Cénit"      d="Si no estás satisfecho con el resultado, repetimos el servicio sin costo."/>
        </div>
      </div>
    </div>
  );
