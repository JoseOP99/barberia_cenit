/* global React, Icon, Sunburst, Corners, CENIT_DATA, formatCOP, romanDate, ROMAN */
const { useState: useStateRes, useMemo: useMemoRes } = React;

// ============================================================
// RESERVAR — Roman numeral stepper, art-deco vibe
// ============================================================
function ViewReservar({ preselectService }) {
  const STEPS = ["Servicio", "Maestro", "Día y Hora", "Confirmación"];
  const [step, setStep] = useStateRes(0);
  const [service, setService] = useStateRes(preselectService ? CENIT_DATA.services.find(s => s.id === preselectService) : null);
  const [barber, setBarber] = useStateRes(null);
  const [date, setDate] = useStateRes(null);
  const [time, setTime] = useStateRes(null);
  const [contact, setContact] = useStateRes({ name: "", phone: "", email: "" });
...
window.ViewReservar = ViewReservar;
...
function StepDateTime({ date, time, onDate, onTime }) {
  const [month, setMonth] = useStateRes(() => new Date(2026, 4, 1));
  const today = new Date(2026, 4, 20);

  const days = useMemoRes(() => {
          ...
          <div className="frame-thin p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              <div className="text-center">
                ...
                <div className="font-mono text-[10px]" style={{ color: '#8B6F3F' }}>{toRoman(month.getFullYear())}</div>
              </div>
              <button onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() + 1, 1))}
...
function Field({ label, value, onChange, placeholder, type = "text" }) {
  return (
    <div>
      ...
      <input className="input-line mt-1" type={type} placeholder={placeholder}
