...
export default function Shop({ onAddToCart = () => {} }) {
  ...
  const colors = [...new Map(CENIT_DATA.products.map(p => [p.colorName || p.color, p.color])).entries()];
...
function FilterRail({ filters, setFilters, toggle, collections, colors, className = "" }) {
  return (
    <div className={className} style={{ borderColor: '#2A2530' }}>
      <div className="mb-8">
        <h4 className="font-roman text-[10px] text-gold tracking-[0.2em] mb-4">COLECCIÓN</h4>
        {collections.map(c => {
  ...
  return (
    <button onClick={onOpen}
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="font-mono text-xs mt-1" style={{ color: '#948A78' }}>{p.material?.toUpperCase() || 'ALGODÓN'} · {p.colorName?.toUpperCase() || 'NEGRO'}</div>
      </div>
